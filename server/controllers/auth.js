import bcrypt from "bcryptjs";
import { db } from "../db.js";
import jwt from "jsonwebtoken";

export const register = (req, res) => {
  //Kiem tra tai khoan
  const q = "SELECT * FROM users WHERE email = ? OR usename = ?";

  db.query(q, [req.body.email, req.body.name], (err, data) => {
    if (err) return res.json(err);
    if (data.length) return res.status(409).json("Tài khoản đã tồn tại!");
    // Ma hoa mk
    const salt = bcrypt.genSaltSync(10);
    const hash = bcrypt.hashSync(req.body.password, salt);

    const q = "INSERT INTO users(`usename`, `email`, `password` ) VALUES (?)";
    const values = [req.body.usename, req.body.email, hash];

    db.query(q, [values], (err, data) => {
      if (err) return res.json(err);
      return res.status(200).json("Tao tai khoan thanh cong");
    });
  });
};

export const login = (req, res) => {
  const q = "SELECT * FROM users WHERE usename = ?";

  // Kiểm tra người dùng
  db.query(q, [req.body.usename], (err, data) => {
    if (err) return res.json(err);
    if (data.length === 0)
      return res.status(404).json("Không tìm thấy người dùng");

    // Kiểm trà mk
    const isPasswordCorrect = bcrypt.compareSync(
      req.body.password,
      data[0].password
    );

    if (!isPasswordCorrect)
      return res.status(400).json("Sai tên người dùng hoặc mật khẩu!");

    const token = jwt.sign({ id: data[0].id }, "jwtkey");
    const { password, ...other } = data[0];

    res
      .cookie("access_token", token, {
        httpOnly: true,
      })
      .status(200)
      .json(other);
  });
};

export const logout = (req, res) => {

  res.clearCookie("access_token",{
    sameSite:"none",
    secure:true
  }).status(200).json("Tài khoản đã được đăng xuất")

};
